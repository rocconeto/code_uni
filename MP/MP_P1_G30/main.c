...
//Para cancelar reservas, añadir que si estaba en el array de eliminar, cuando se elimine la reserva también se elimine la habitación del array habitaciones
...
/*Tipo:                     Procedimiento (función tipo void)                                   */
/*Parámetros de entrada:    Arrays habitaciones y clientes y contadores clientes y habitacioens */
        ...
        if(clientes[i].cantReservas != 0){
            for (int j = 0; j < MAX_RESERVAS; j++){
                if(strcmp(habitaciones[encontrado].codigo, clientes[i].reservas[j]) == 0){
                    ...
                    printf("\n\tSe eliminará automáticamente cuando no posea reservas asociadas\n\n");

                    //Cuando tenga lo de cancelar reservas se eliminará automáticamente
                    return;
                }
            }
        }
...
/*Tipo:                     Procedimiento (función tipo void)                                   */
/*Parámetros de entrada:    Array clientes, array reservas y contadores de clientes y reservas  */
/*Parámetros de salida:     Array reservas y contador reservas                                  */
/*Prerrequisitos:           Es necesario que haya espacio para nuevos clientes y que el DNI     */
/*                          introducido no exista en el array                                   */
/*Objetivo:                 Se encarga de añadir nuevas reservas al array de reservas           */
/************************************************************************************************/
...
void guardar_reservas(char reservas[DIAS_MES][MAX_RESERVAS][8]){
    ...
    for(int dia = 0; dia < DIAS_MES; dia++){ // Corrección en el incremento de dia++
        if(reservas[dia][0][0] != '\0'){ // Si hay al menos una reserva en este día
            fprintf(reserv, "%d\n", dia + 1); // Corrección en el formato del día

            for(int i = 0; i < MAX_RESERVAS; i++){
                if(reservas[dia][i][0] != '\0'){ // Si hay una reserva en esta posición
                    fprintf(reserv, "%s ", reservas[dia][i]); // Separación con espacio
                }
            }
            fprintf(reserv, "\n"); // Nueva línea para el siguiente día con reservas
        }
    }
